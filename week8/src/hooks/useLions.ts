...
interface UseLionsReturn {
  members: Lion[];
  nextId: number;
  addMember: (formData: AddMemberData) => void;
...
export function useLions(): UseLionsReturn {
  const [members, setMembers] = useState<Lion[]>(initialLions);
  const [nextId, setNextId] = useState<number>(
    Math.max(...initialLions.map((m) => m.id)) + 1
  );

  function addMember(formData: AddMemberData): void {
    const newMember: Lion = {
      id: nextId,
      ...formData,
      isMe: false,
